...
# Apply your professional Liberation Serif layout configurations
plt.rcParams.update({
    'font.family': 'serif',
...
def run_3way_reproducible_benchmark():
    NUM_RUNS = 10
    # OPTIMIZATION: Scaled to 600 nodes max to secure immediate, clean results
    ...
    for num_nodes in node_counts:
        ...
        # --- INCREMENTAL PLOTTING (Moved inside the loop to secure data points) ---
        current_progress_nodes = node_counts[:len(final_results[LABEL_QQAR_2HOP]['pdr'])]
        ...
        for agent in agents_list:
            axs[0, 0].errorbar(current_progress_nodes, final_results[agent]['pdr'], yerr=final_std[agent]['pdr'], marker=markers[agent], color=colors[agent], label=agent, linewidth=2, capsize=3)
        ...
        # Delay
        for agent in agents_list:
            axs[0, 1].errorbar(current_progress_nodes, final_results[agent]['delay'], yerr=final_std[agent]['delay'], marker=markers[agent], color=colors[agent], label=agent, linewidth=2, capsize=3)
        axs[0, 1].set(title='(b) Average E2E Delay vs WBANs', xlabel='Number of WBANs', ylabel='Delay (ms)')
        axs[0, 1].legend()

        # Routing Overhead
        for agent in agents_list:
            axs[1, 0].errorbar(current_progress_nodes, final_results[agent]['ro'], yerr=final_std[agent]['ro'], marker=markers[agent], color=colors[agent], label=agent, linewidth=2, capsize=3)
        axs[1, 0].set(title='(c) Routing Overhead vs WBANs', xlabel='Number of WBANs', ylabel='RO (Bytes)')
        axs[1, 0].legend()

        # Energy Consumption 
        x = np.arange(len(current_progress_nodes))
        width = 0.25
        ...
        axs[1, 1].set_xticklabels(current_progress_nodes)
        axs[1, 1].legend()
        ...
        # Change this line in your NEW renamed script:
        save_path = os.path.join(save_dir, 'algorithm_3way_comparison_truncated.png')
        plt.savefig(save_path, dpi=300)
        plt.close()
        print(f"--> Saved intermediate progress graph to: {save_path}")
